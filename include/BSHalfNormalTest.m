...
lambdaBSsort=sort(est.lambdaBS-mean(est.lambdaBS),2,'ascend');

% pairwise decisions and p-value
for p1=1:1:P-1
    for p2=p1+1:P
        VarDeltasort(p1,p2)=var(lambdaBSsort(:,p1)-lambdaBSsort(:,p2));
        test.SBE(p1,p2) = sqrt(VarDeltasort(p1,p2))/sqrt(1-2/pi);
