...
test.lambdaBSsort=sort(est.lambdaBS-mean(est.lambdaBS),2,'ascend');

% pairwise decisions and p-value
for p1=1:1:P-1
    for p2=p1+1:P
        test.VarDeltasort(p1,p2)=var(test.lambdaBSsort(:,p1)-test.lambdaBSsort(:,p2));
        test.SBEsort(p1,p2) = sqrt(test.VarDeltasort(p1,p2))/sqrt(1-2/pi);
